...
app.config['PROFILE_PICS_FOLDER'] = os.path.join(app.config['UPLOAD_FOLDER'], 'prof_pics')
app.config['BOOK_PICS_FOLDER'] = os.path.join(app.config['UPLOAD_FOLDER'], 'books_pics')
...
CORS(app, resources={r"/*": {"origins": "http://127.0.0.1:50814"}})
...
def manage_profile():
    ...
    if request.method == 'GET':
        ...
        if file and file.filename:
            ...
            print("File path to be saved:", file_path)  # Debugging print

        db.session.commit()

        # Additional debugging to confirm the update
        updated_user = User.query.get(current_user_id)
        print("Updated profile picture path in DB:", updated_user.profile_pic)
...
def add_book():
    ...
    # Assume data is sent as multipart/form-data
    data = request.form
    ...
    try:
        ...
        loan_type = int(data['loan_type'])  # Convert loan_type to integer if necessary
        ...
        new_book = Book(
            ...
            loan_type=loan_type,
            active=data.get('active', 'active'),  # Default to 'active'
            status=data.get('status', 'available')  # Default to 'available'
        )

        print("Created new book:", new_book)

        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            save_path = os.path.join(app.config['BOOK_PICS_FOLDER'], filename)
            file.save(save_path)
            new_book.book_pic = save_path  # Save the path to the book picture in the database
...
def update_book(id):
    ...
    # Using request.form and request.files to handle form-data and file uploads
    data = request.form
    file = request.files.get('book_pic')
    ...
    try:
        ...
        return jsonify({"message": "Invalid integer fields"}), 400

    if file and allowed_file(file.filename):
        filename = secure_filename(file.filename)
        save_path = os.path.join(app.config['BOOK_PICS_FOLDER'], filename)
        file.save(save_path)
        book.book_pic = save_path  # Update the file path
